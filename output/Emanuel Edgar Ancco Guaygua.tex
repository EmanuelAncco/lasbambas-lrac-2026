...
% --- Fuente: Times New Roman real via XeLaTeX ---
\usepackage{fontspec}
...
% --- Mejor formato de tablas: lineas finas y espaciado consistente ---
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Diagnóstico y Gobernanza del Sistema LRAC}
\fancyhead[R]{\small E. Ancco · Innovadores en Acción 2026}
...
\newtcolorbox{kbox}[1][]{
  colback=white, colframe=black, boxrule=0.5pt, arc=2pt, breakable,
...
\textbf{Destinatario:} \texttt{Peru.Recruitment@MMG.COM}\\[0.5cm]

\small\textit{Anexos digitales (ver Sección 8):}\\
\small\href{https://github.com/EmanuelAncco/lasbambas-lrac-2026}{github.com/EmanuelAncco/lasbambas-lrac-2026} \,$\cdot$\,
\href{https://lasbambas-lrac-emanuel.streamlit.app}{lasbambas-lrac-emanuel.streamlit.app}\\[0.5cm]
...
\textbf{M1.B} & \textbf{TOKI} (n8n + Evolution API + Gemini Pro + Postgres pgvector) & Bot WhatsApp con \textbf{50 nodos n8n}, \textbf{10 herramientas} integradas, memoria persistente, procesa texto/audio/foto/PDF & L · A · C & \textit{producción} \\
...
\item \textbf{M1.A · Visión computacional EPP} habilita \emph{A-05} (verificación independiente CCV vía cámaras), \emph{A-12} (cobertura EPP $\geq$99\,\% en accesos y $\geq$97\,\% en obra) y \emph{A-25} (wearables).
\item \textbf{M1.B · TOKI} habilita \emph{A-01} (medición 3Q VP SHE), \emph{A-02} (Tarjeta Verde digital), \emph{A-08} (cápsula semanal con quiz), \emph{A-10} (onboarding asistido), \emph{A-11} (plataforma RACS conversacional, Figura \ref{fig:whatsapp}) y \emph{A-17} (BBS dirigido).
\item \textbf{M1.C · GAIATECH VIGÍA} habilita \emph{A-13} (monitoreo estructural relaves alineado al DS 034 art.\ 418--423).
\item \textbf{M1.D · Dashboard ejecutivo} habilita \emph{A-06} (tablero ICAM), \emph{A-14} (Single Pane of Glass), \emph{A-15} (API IEM bidireccional), \emph{A-22} (ranking público) y \emph{A-24} (predicción ML).
\end{itemize}
\end{minipage}\hfill
\begin{minipage}{0.34\linewidth}
\centering
\includegraphics[width=\linewidth]{../figures/fig17_whatsapp_mockup.png}
\captionof{figure}{Workflow LRAC-001 · captura RACS conversacional vía TOKI.}
\label{fig:whatsapp}
\end{minipage}
\vspace{6pt}
...
\item Video de 90 segundos mostrando el bot TOKI capturando un RACS en vivo y la detección EPP en producción.
...
\begin{enumerate}
\item \textbf{Pacto público del custodio (SHE).} El VP SHE firma y comunica un \emph{Pacto 3Q} en el que se compromete personalmente al cumplimiento del 90\,\% de 3Q dentro de su propia VP en los próximos 60 días. KPI: 3Q SHE $\geq 90\,\%$ a junio 2026 (medido por bot TOKI en planta).
...
\item \textbf{Universidad LRAC interna} (TOKI + LMS) con rutas formativas por rol y certificación digital.
...
\input{../analysis/tabla_acciones.tex}
...
\input{../analysis/tabla_presupuesto.tex}
...
\input{../analysis/tabla_riesgos.tex}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\linewidth}{@{}p{0.8cm} >{\raggedright\arraybackslash}p{3.4cm} X@{}}
\toprule
\textbf{Anexo} & \textbf{Entregable} & \textbf{Descripción y URL} \\
\midrule
\textbf{A1} & Repositorio público en GitHub & Código Python reproducible del análisis estadístico, scripts de visualización, documentación arquitectónica de los 12 workflows GAIATECH M1.0, JSON exportables de 4 workflows y plantilla LaTeX del informe. Licencia MIT. \newline \url{https://github.com/EmanuelAncco/lasbambas-lrac-2026} \\
\textbf{A2} & Demo Streamlit interactivo & Tres páginas que replican el dashboard ejecutivo LRAC sobre los datos del caso Mina Juanita: (i) filtros VP\,/\,Gerencia\,/\,Mes con evolución, heatmap y ranking dinámicos; (ii) simulador del plan con sliders por pilar y trayectoria proyectada a 24 meses; (iii) catálogo de los 12 workflows GAIATECH M1.0. \newline \url{https://lasbambas-lrac-emanuel.streamlit.app} \\
\textbf{A3} & Video 90 s GAIATECH M1.0 & Demostración audiovisual del bot TOKI capturando un RACS en vivo (texto, foto y voz), detección EPP YOLOv8 continua y dashboard ejecutivo Vision Pro PDK. Voz Gemini TTS Kore. Subido a YouTube unlisted; URL incluida en el correo de envío. \\
\textbf{A4} & Slides 10 min sustentación oral & Deck Slidev (14 slides, HTML público + PDF descargable) para etapa 2 si aplica. Misma narrativa que el PDF, condensada. URL incluida en el correo de envío. \\
\textbf{A5} & Dashboard HTML equivalente a Power BI & \texttt{report/Emanuel\_LRAC\_Dashboard.html}: cinco pestañas Plotly (resumen, dashboard, pilares, simulador, detalle gerencia × mes). Replica funcionalmente el \texttt{.pbix} sin requerir licencia Power BI por parte del jurado; abre en cualquier navegador. \\
\bottomrule
\end{tabularx}
\caption{Anexos digitales del informe. Cada uno está alineado a un módulo GAIATECH M1.0; no son ornamento, son la evidencia operativa de que el postulante construye, no solo describe.}
\label{tab:anexos}
\end{table}

Cada anexo se alinea a un módulo GAIATECH M1.0 (A1 reproducibilidad; A2 prefigura M1.D; A3 demuestra M1.A+M1.B+M1.D en vivo; A4 condensa la narrativa; A5 entrega un activo reutilizable). El portafolio técnico complementario está en \href{https://emanuelancco.github.io/cv-portfolio/}{emanuelancco.github.io/cv-portfolio}. Adicionalmente, una instancia operativa del workflow LRAC-001 está desplegada en n8n para inspección directa por parte del jurado: \url{https://aecodetest.app.n8n.cloud/workflow/qRW88FSByN26nDJr}.
