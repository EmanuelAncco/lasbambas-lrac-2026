---
titulo: "Modeling Mine Workforce Fatigue: Finding Leading Indicators of Fatigue in Operational Data Sets"
autores:
  - (Autores University of Utah, Mining Engineering + Psychology - véase MDPI Minerals)
revista: Minerals (MDPI)
year: 2021
volumen: 11
numero: 6
articulo: 621
doi: 10.3390/min11060621
url: https://www.mdpi.com/2075-163X/11/6/621
tema: Fatiga minera, indicadores líderes vs. rezagados, machine learning, salud y seguridad
citacion: "Modeling Mine Workforce Fatigue: Finding Leading Indicators of Fatigue in Operational Data Sets. (2021). Minerals, 11(6), 621. https://doi.org/10.3390/min11060621"
---

# Indicadores Líderes de Fatiga Minera (Minerals MDPI, 2021)

## Abstract resumido

Explora cómo los conjuntos de datos operacionales disponibles en minas modernas (operación, salud y seguridad, salud del equipo, monitoreo de fatiga, clima) pueden modelarse con machine learning para identificar indicadores líderes de fatiga del personal. Confirma la relación de la fatiga con la hora del día y las horas trabajadas, y posiciona el ML como herramienta para que la gerencia tome mejores decisiones preventivas.

## Hallazgos clave aplicables

- **Distinción leading vs. lagging**: los indicadores rezagados (tasas de lesión) son útiles para auditoría pero no previenen; los líderes (condiciones del proceso) son los que importan al M1.0. Concepto clave para defender el dashboard predictivo.
- **Variables operativas como insumo de ML**: horas trabajadas, turnos, clima, salud del equipo. Todas son disponibles en MMG Las Bambas y pueden integrarse vía n8n.
- **Mining engineering + psychology**: combinación interdisciplinaria que valida la inclusión del módulo conversacional TOKI (componente "humano" del M1.0).
- **Fatiga como riesgo cuantificable**: argumento para que GAIATECH M1.0 incluya un score predictivo de fatiga en el dashboard.
- **Caso de uso operativo**: los datos ya existen en los mines modernos; el desafío es integrarlos. El n8n + Postgres del M1.0 resuelve la integración.

## Cómo apoya la propuesta GAIATECH M1.0

Justifica añadir indicadores líderes (no solo lagging) en el dashboard del informe LRAC. Argumenta el porqué de un módulo de fatiga predictiva alimentado por turnos, clima y reportes TOKI. Convierte al M1.0 en una herramienta de prevención y no solo de monitoreo, alineado con la doctrina de seguridad moderna que MMG Las Bambas exige.

## Citación APA 7

Modeling Mine Workforce Fatigue: Finding Leading Indicators of Fatigue in Operational Data Sets. (2021). *Minerals*, *11*(6), 621. https://doi.org/10.3390/min11060621
